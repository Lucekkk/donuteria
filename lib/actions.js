...
export async function registerUser(prevState, formData) {
  ...
  const loginTaken = await users.some((user) => user.login === newUser.login.trim());
  if (loginTaken) {
    return { 
  ...
  const emailTaken = await users.some((user) => user.email === newUser.email.trim());
  if (emailTaken) {
    return { 
...
export async function loginUser(prevState, formData) {
  ...
  if(!foundUser){
    ...
    const valid = await argon2.verify(foundUser.password_hash, loggedUser.password_hash);
