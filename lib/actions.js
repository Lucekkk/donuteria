...
import { editUserAuth } from "@/app/api/editUserAuth/route";
...
export async function loginUser(prevState, formData) {
  ...
  if (foundUser.czyZbanowany === 1) {
    return {
      message: "Zbanowany",
      banReason: foundUser.powod_bana || "Brak podanego powodu",
      values: {
        email: loggedUser.email || "",
        password_hash: "",
        isChecked: loggedUser.isChecked || false,
...
export async function cartSummary(prevState, formData) {
  const clientFormOrder = {
    ...
    orderPrice: (formData.get("orderPrice") * 1).toFixed(2) * 1,
...
export async function editUserAccount(prevState, formData) {
  ...
  if (
    isInvalidText(editAuthForm.login) ||
    isInvalidText(editAuthForm.password_hash) ||
    isInvalidText(editAuthForm.confirmPassword)
  ) {
    return {
      message: "Uzupełnij dane",
      values: {
        login: editAuthForm.login || "",
        password_hash: editAuthForm.password_hash || "",
        confirmPassword: editAuthForm.confirmPassword || "",
      },
    };
  }

  if (editAuthForm.password_hash !== editAuthForm.confirmPassword) {
    return {
  ...
  if (!isValidLogin(editAuthForm.login)) {
    return {
      message: "Login musi mieć długość między 1 a 35 znaków",
      values: {
        login: editAuthForm.login || "",
  ...
  const loginTaken = await users.some(
    (user) =>
      user.id !== editAuthForm.userID &&
      user.login === editAuthForm.login.trim(),
  );
  if (loginTaken) {
    return {
      message: "Nazwa loginu jest już zarezerowana",
  ...
  if (!isValidPassword(editAuthForm.password_hash)) {
    return {
      message:
        "Hasło musi mieć od 8 do 20 znaków, oraz posiadać jedną cyfrę i jedną, wielką literę",
      values: {
  ...
  const hash = await argon2.hash(editAuthForm.password_hash, {
    type: argon2.argon2id,
    memoryCost: 65536,
    timeCost: 3,
    parallelism: 1,
  ...
  editAuthForm.password_hash = hash;

  await editUserAuth(editAuthForm);
  redirect(`/profil/${editAuthForm.userID}`);
}
